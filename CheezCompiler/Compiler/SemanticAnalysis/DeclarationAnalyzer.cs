...
namespace Cheez.Compiler.SemanticAnalysis
{
    class DeclarationAnalyzer
    {
        ...
        public void CollectDeclarations(Workspace ws, IErrorHandler errorHandler)
        {
            ...
            //
            RawAstPrinter printer = new RawAstPrinter(Console.Out);

            Console.WriteLine("--------------------------");
            Console.WriteLine("structs");
            foreach (var f in mStructs)
            {
                printer.PrintStatement(f);
            }

            Console.WriteLine("--------------------------");
            Console.WriteLine("poly structs");
            foreach (var f in mPolyStructs)
            {
                printer.PrintStatement(f);
            }
        ...
        private void Pass1TypeAlias(AstTypeAliasDecl t)
        {
