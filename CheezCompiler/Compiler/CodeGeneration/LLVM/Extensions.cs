...
namespace Cheez.Compiler.CodeGeneration.LLVMCodeGen
{
    ...
    {
        public static void AddFunctionAttribute(this LLVMValueRef self, LLVMContextRef context, AttributeKind kind, uint value = 0)
        {
        ...
        public static void AddFunctionReturnAttribute(this LLVMValueRef self, LLVMContextRef context, AttributeKind kind, uint value = 0)
        {
            var att = LLVM.CreateEnumAttribute(context, kind.ToUInt(), value);
            LLVM.AddAttributeAtIndex(self, LLVMAttributeIndex.LLVMAttributeReturnIndex, att);
        }

        public static void AddFunctionParamAttribute(this LLVMValueRef self, LLVMContextRef context, int param, AttributeKind kind, uint value = 0)
        {
            var att = LLVM.CreateEnumAttribute(context, kind.ToUInt(), value);
            LLVM.AddAttributeAtIndex(self, (LLVMAttributeIndex)(param + 1), att);
        }

        public static void AddCallAttribute(this LLVMValueRef self, LLVMContextRef context, int arg, AttributeKind kind, uint value = 0)
        {
            var att = LLVM.CreateEnumAttribute(context, kind.ToUInt(), value);
